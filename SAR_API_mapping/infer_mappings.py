...
jackson_embeddings_path = "./embeddings/vectors-Jackson.txt"
gson_embeddings_path = "./embeddings/vectors-Gson.txt"

with open(jackson_embeddings_path) as cs_f:
    # next(cs_f)
    cs_embeddings = cs_f.readlines()

with open(gson_embeddings_path) as java_f:
    # next(java_f)
...
for cs_emb in cs_embeddings:
    ...
    if func(split[0]) == True:
        if check_package_include(jackson_packages, split[0]) == True:
            # if split[0][0] == "S":
...
for java_emb in java_embeddings:
    ...
    if func(split[0]) == True:
        ...
        if check_package_include(gson_packages, split[0]) == True:
...
jackson_vectors = KeyedVectors.load_word2vec_format(jackson_embeddings_path, binary=False)
gson_vectors = KeyedVectors.load_word2vec_format(gson_embeddings_path, binary=False)
...
for cs_token in cs_signature_tokens:
    ...
    k_nearest = gson_vectors.similar_by_vector(jackson_vectors[cs_token], topn=top_k)
    relevant_k = list()
    for k in k_nearest:
        if func(k[0]) == True:
    ...
    if is_exact_name:
        outpur_cs_url = "./usage_mapping/" + project + "_" + usage_type + "_usage_mapping_cs_exact_name_" + str(
            top_k) + ".txt"
    else:
        outpur_cs_url = "./usage_mapping/" + project + "_" + usage_type + "_usage_mapping_cs_" + str(
            top_k) + "_cos_lr_s_1" + ".txt"
    if len(relevant_k) != 0:
...
for java_token in java_signature_tokens:
    ...
    method_source = split[len(split) - 1].split("(")[0]
    k_nearest = jackson_vectors.similar_by_vector(gson_vectors[java_token], topn=top_k)
    relevant_k = list()
    for k in k_nearest:
        if func(k[0]) == True:
    ...
    if is_exact_name:
        outpur_java_url = "./usage_mapping/" + project + "_" + usage_type + "_usage_mapping_java_exact_" + str(
            top_k) + ".txt"
    else:
        outpur_java_url = "./usage_mapping/" + project + "_" + usage_type + "_usage_mapping_java_" + str(
            top_k) + "_cos_lr_s_1" + ".txt"
