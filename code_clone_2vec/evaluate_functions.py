...
from util import mean_reciprocal_rank
...
with open("./evaluation_data/functions/functions_" + PROJECT + "_3.csv","r") as f_csv:
	reader = csv.reader(f_csv)
	java_phrases = list()
	cs_phrases = list()
	mapping_java = {}
	...
	for a in range(15):
		...
		print ("CS part ....")
		for i,cs_k in tqdm(enumerate(random_cs_phrases)):
			# print "############################"
			
			try:
				
				top_k_java_average, top_k_java_average_tfidf = get_top_k_match(10,cs_k,java_phrases,cs_vectors,java_vectors)
		...
		map_cs_average = "MAP CS average: " + str(mean_reciprocal_rank(avg_p_list_cs_average))
		map_cs_average_tfidf = "MAP CS average tfidf: " + str(mean_reciprocal_rank(avg_p_list_cs_average_tfidf))
		# print "MAP CS sum: " + str(mean_reciprocal_rank(avg_p_list_cs_sum))

		with open("./evaluation_result/function_result_average_" + PROJECT + "_11_20_1" + ".csv","a") as f:
		...
		for i,java_k in tqdm(enumerate(random_java_phrases)):
			...
			try:
				...
				top_k_cs_average,top_k_cs_average_tfidf = get_top_k_match(10,java_k,cs_phrases,java_vectors,cs_vectors)
				# print top_5_cs
				relevant_list_average = list()
				relevant_list_average_tfidf = list()
				relevant_list_sum = list()
		...
		map_java_average =  "MAP Java average : " + str(mean_reciprocal_rank(avg_p_list_java_average))
		map_java_average_tfidf = "MAP Java average tfidf: " + str(mean_reciprocal_rank(avg_p_list_java_average_tfidf))
